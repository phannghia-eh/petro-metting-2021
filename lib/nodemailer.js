    ...
    static transporter() {
        return nodemailer.createTransport({
            ...
            auth: {
                ...
                user: 'b95579db5b69a995f045c106fc18b754', // phn1910@gmail.com gmail
                    ...
                    <tbody>
                      <tr>
                        <td align="left" style="font-size:0px;padding:10px 25px;padding-top:0px;padding-bottom:0px;word-break:break-word;">
                          <div style="font-family:Arial, sans-serif;font-size:13px;letter-spacing:normal;line-height:1;text-align:left;color:#000000;">
                            <p class="text-build-content" data-testid="7m0I-YxI_1a" style="margin: 10px 0; margin-top: 10px; margin-bottom: 10px;">Vui lòng đặt tên tham dự như sau: <b>${passcode}</b>
                            </p>
                          </div>
                        </td>
                      </tr>
                    ...
                    <tbody>
                      <tr>
                        <td align="left" style="font-size:0;padding:10px 25px;padding-top:0;padding-bottom:0;word-break:break-word">
                          <div style="font-family:Arial,sans-serif;font-size:13px;letter-spacing:normal;line-height:1;text-align:left;color:#000">
                            <p class="text-build-content" data-testid="7m0I-YxI_1a" style="margin:10px 0;margin-top:10px;margin-bottom:10px">Thông tin đăng ký của bạn không phù hợp với buổi họp, vui lòng liên hệ với phòng ban tổ chức để xác nhận lại thông tin</p>
                          </div>
                        </td>
                      </tr>
                    </tbody>
...
\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<![endif]-->
    ...
    static async sendSuccessMail(data) {
        ...
        return new Promise(async (resolve, reject) => {
            this.transporter().sendMail(this.buildMail(data.email, this.successMailTemplate(data.email, data.passcode)), (err, info) => {
    ...
    static async sendFailMail(data) {
        ...
        return new Promise(async (resolve, reject) => {
            this.transporter().sendMail(this.buildMail(data.email, this.failMailTemplate(data.email)), (err, info) => {
                if (err) return reject(err)
                return resolve(info)
            })
        })
